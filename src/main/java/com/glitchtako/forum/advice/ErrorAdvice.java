...
public class ErrorAdvice implements ResponseBodyAdvice<RestResponse<?>> {
  ...
  public RestResponse<?> exceptionHandler(
    ...
    log.error(
        ...
        req.getRequestURL());
    return RestResponse.fail(
        ErrorCode.METHOD_NOT_ALLOWED.code, ex.getMessage(), ErrorCode.METHOD_NOT_ALLOWED.status);
  }

  @ExceptionHandler(value = HttpClientErrorException.Forbidden.class)
  public RestResponse<?> exceptionHandler(
      HttpServletRequest req, HttpClientErrorException.Forbidden ex) {
    log.error(
  ...
  public RestResponse<?> exceptionHandler(HttpServletRequest req, BadCredentialsException ex) {
    log.error(
        ...
        req.getRequestURL());
    return RestResponse.fail(
        ErrorCode.UNAUTHORIZED.code, ex.getMessage(), ErrorCode.UNAUTHORIZED.status);
  }
  ...
  @Override
  public RestResponse<?> beforeBodyWrite(
      RestResponse<?> body,
    ...
    if (body.getHttpStatus() != null) response.setStatusCode(body.getHttpStatus());
